...
namespace vidu8 {
// Ví dụ 8: Minh họa vẽ cảnh đơn giản và thao tác di chuyển của người

float lx = 0.0f, ly = 0.0f;
float x = 0.0f, y = 0.0f;
float angle = 0.f;

LinedModel pyramid, tile, cone, cone1;
...
void drawGround() {
	...
	for (int x = -100; x < 100; x += 1) {
		for (int y = -100; y < 100; y += 1) {
			//drawLinedModel(&tile, point3(x, y, 0));
...
void drawObjects() {
	...
	for (int x = -50; x < 50; x += 7) {
		for (int y = -50; y < 50; y += 7) {
			...
			if (idxColor % 2 == 0) drawLinedModel(&pyramid, point3(x, y, 0));
			else drawLinedModel(&cone1, point3(x, y, 0));
...
void display(void) {
	...
	//glColor3f(0., 1., 0.);//RGB
	//drawLinedModel(&pyramid);
	//drawLinedModel(&cone1);
